...
const getAccessTokenWithRetry = async function(issuer, authorizationCode, retryCount) {
  if(!retryCount){
    retryCount = 0;
  }

  try {
    return await getAccessToken(issuer, authorizationCode);
  } catch(e) {
    if(retryCount < requestRetries) {
      const newRetryCount = retryCount + 1;
      console.log(`${e}, Retry #${newRetryCount}.`);
      return await getAccessTokenWithRetry(issuer, authorizationCode, newRetryCount);
    }else {
      throw e;
    }
  }
