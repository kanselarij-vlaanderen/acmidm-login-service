...
import { ovoCodeFromVoEmail, ensureOrganization } from './organization';
...
const ensureOrganisationForUser = async function (personUri, claims, graph) {
  /*
   * The vo_orgcode claim currently doesn't come through. Since it looks like often times the OVO-code
   * is included in the vo_email claim, we parse it from there for now.
   */
  const ovoCode = claims[voEmailClaim] && ovoCodeFromVoEmail(claims[voEmailClaim]);
  if (!ovoCode) {
    return;
  }
  const queryString = `
