...
import { GROUP_GRAPH_URI } from './session';
...
const phoneClaim = process.env.MU_APPLICATION_AUTH_VO_PHONE_CLAIM || 'phone_number';
const roleClaim = process.env.MU_APPLICATION_AUTH_ROLE_CLAIM;

const USER_GRAPH_URI = process.env.USER_GRAPH_URI || process.env.MU_APPLICATION_GRAPH;
...
const voPhoneToPhone = function (voPhone) {
  return voPhone.split(':')[0];
};
...
const insertNewUser = async function (claims, graph) {
  ...
  if (claims[phoneClaim]) {
    const phoneUri = phoneToUri(sanitizePhoneNumber(voPhoneToPhone(claims[phoneClaim])));
...
const checkIfPhoneShouldbeUpdated = (result, claims) => !testIfPropertyUpToDate(result.phone && result.phone.value && uriToPhone(result.phone.value), claims[phoneClaim] && sanitizePhoneNumber(voPhoneToPhone(claims[phoneClaim])));
...
const selectUserGroup = async function (accountUri, claims, roleClaim, userGraphUri) {
...
const addUserToOrganization = async function (userUri, organizationUri, userGraphUri) {
...
const ensureOrganisationForUser = async function (personUri, ovoCode, orgGraphUri, userGraphUri) {
  const queryString = `
    PREFIX org: <http://www.w3.org/ns/org#>
  ...
  if (queryResult.results.bindings.length) {
    ...
    const organization = await ensureOrganization(ovoCode, orgGraphUri);
    await addUserToOrganization(personUri, organization.organizationUri, userGraphUri);
  }
};

const ensureOrganisationsForUser = async function (personUri, claims, orgGraphUri, userGraphUri) {
  /*
  ...
  if (claims[roleClaim]) {
    for (const group of claims[roleClaim]) {
      ...
      await ensureOrganisationForUser(personUri, ovoCode, orgGraphUri, userGraphUri);
    }
  }
};
