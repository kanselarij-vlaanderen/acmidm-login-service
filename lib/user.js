...
import { RESOURCE_BASE_URI, USERS_GRAPH, AUTH_USERID_CLAIM, AUTH_FIRST_NAME_CLAIM, AUTH_FAMILY_NAME_CLAIM, ACCESS_BLOCKED_STATUS_URI, ACCESS_ALLOWED_STATUS_URI } from '../config';
  ...
  if (userIdentifier) {
    ...
    if (queryResult.results.bindings.length) {
      ...
      if (status === ACCESS_BLOCKED_STATUS_URI) {
        throw new BlockedError(`The user has been blocked and does not have access.`)
      }
...
const ensureFreshUserData = async function (person, claims) {
  const properties = [
    ...
    { predicate: 'adms:status', newValue: ACCESS_ALLOWED_STATUS_URI },
  ];
