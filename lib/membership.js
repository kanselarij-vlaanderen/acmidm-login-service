...
const blockMembership = async function(membershipUri) {
  await update(`
  PREFIX adms: <http://www.w3.org/ns/adms#>

  DELETE WHERE {
    GRAPH <${USERS_GRAPH}> {
      ${sparqlEscapeUri(membershipUri)} adms:status ?status .
    }
  }`);
  await update(`
  PREFIX adms: <http://www.w3.org/ns/adms#>
  ...
  INSERT DATA {
    GRAPH <${USERS_GRAPH}> {
      ${sparqlEscapeUri(membershipUri)} adms:status ${sparqlEscapeUri(ACCESS_BLOCKED_STATUS_URI)} .
    }
  }`);
}
