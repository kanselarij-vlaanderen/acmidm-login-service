...
const emailClaim = process.env.MU_APPLICATION_AUTH_VO_EMAIL_CLAIM || 'vo_email';
const orgCodeClaim = process.env.MU_APPLICATION_AUTH_VO_ORGCODE_CLAIM || 'vo_orgcode';
const phoneClaim = process.env.MU_APPLICATION_AUTH_PHONE_CLAIM || 'phone';
...
const checkIfAccountShouldBeUpdated = function (result, claims) {
  if(!result) {
    return false;
  }
...
const ensureAccountForUser = async function (personUri, claims, graph) {
  ...
  const queryResult = await query(`
    ...
    SELECT ?account ?accountId ?email
    FROM <${graph}> {
  ...
  if (queryResult.results.bindings.length) {
    ...
    if(checkIfAccountShouldBeUpdated(result,claims)) {
...
const insertNewAccountForUser = async function (person, claims, graph) {
  ...
  if (claims.vo_email) {
    insertData += `${sparqlEscapeUri(person)} ext:email ${sparqlEscapeString(claims.vo_email)} . `;
  }
  insertData += `
      }
    }
