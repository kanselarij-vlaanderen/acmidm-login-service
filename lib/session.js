...
const updateMetadataForAccount = async function (accountUri, USER_GRAPH_URI) {
  const now = new Date();
  ...
  INSERT DATA {
    GRAPH <${USER_GRAPH_URI}> {
      ${sparqlEscapeUri(accountUri)} ext:lastLogin ${sparqlEscapeDateTime(now)} .
    }
  }`;
