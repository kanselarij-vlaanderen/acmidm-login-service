...
import { blockMembership, ensureMembership } from './membership';
...
const ensureUserResources = async function (claims, roleUri) {
  const { organizationUri, organizationStatus } = await ensureOrganization(claims);
  ...
  if (organizationStatus === ACCESS_BLOCKED_STATUS_URI) {
    await blockMembership(membershipUri);
    throw new BlockedError(`This organization is blocked`);
  }
