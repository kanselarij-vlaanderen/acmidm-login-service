...
const checkIfEmailShouldBeUpdated = function (result, claims) {
  if(!result) {
    return false;
  }

  if(result.email) {
    return result.email.value === claims.vo_email;
  }

  return false;
};

const checkIfPhoneShouldbeUpdated = function(result, claims) {
  if(!result) {
    return false;
  }

  if(result.phone) {
    return result.phone.value === claims.phone;
  }
  ...
  const queryResult = await query(`
    ...
    FROM <${graph}> {
      ...
      ?account a foaf:OnlineAccount ;
              ...
              OPTIONAL {
                 ?account ext:email ?email
              }
              OPTIONAL {
                 ?account ext:phone ?phone
              }
    }`);

  //TODO: Optional om te checken of email reeds ingevuld is of verschillend is van de email in claims

  if (queryResult.results.bindings.length) {
    const result = queryResult.results.bindings[0];

    if(checkIfEmailShouldBeUpdated(result,claims)) {
      await updateUserAccount(result.account.value,claims,graph);
    }

    if(checkIfPhoneShouldbeUpdated(result,claims)) {
      await updateUserAccount(result.account.value, claims,graph);
    }
...
const updateUserAccount = async function (accountUri, claims, graph) {
  let updateQuery = `
    PREFIX foaf: <http://xmlns.com/foaf/0.1/>
    PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
    PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    ...
    DELETE {
      GRAPH <${graph}> {
        ${sparqlEscapeUri(accountUri)}  ext:email ?email .
      }
    } WHERE {
      GRAPH <${graph}> {
        ${sparqlEscapeUri(accountUri)} a foaf:OnlineAccount ;
        OPTIONAL {
          ${sparqlEscapeUri(accountUri)} mu:uuid ?accountId .
        }
        OPTIONAL {
          ${sparqlEscapeUri(accountUri)} ext:email ?email .
        }
         OPTIONAL {
          ${sparqlEscapeUri(accountUri)} ext:phone ?phone .
        }
      }
    }`;
  ...
  INSERT DATA {
    GRAPH <${graph}> {
      ${sparqlEscapeUri(accountUri)} ext:email ${sparqlEscapeString(claims.vo_email)} ;
                                     ext:phone ${sparqlEscapeString(claims.phone)} .
  `;
  updateQuery += `
      }
    }
...
const insertNewAccountForUser = async function (person, claims, graph) {
  ...
  if (claims.vo_email) {
    insertData += `${sparqlEscapeUri(account)} ext:email ${sparqlEscapeString(claims.vo_email)} . `;
  }
  if (claims.phone) {
    insertData += `${sparqlEscapeUri(account)} ext:phone ${sparqlEscapeString(claims.vo_email)} . `;
  }
